...
'Version mismatch between ObjectPlugin and Plugins.pm $Foswiki::Plugins::VERSION. 1.026 required.'
        );
    }
    ...
    Foswiki::Func::registerTagHandler( 'FULLCALENDAR', \&_handleFullCalendar,
        'context-free' );
...
sub _eventsRESTHandler {
    my ($session) = @_;
    return Foswiki::Plugins::ObjectPlugin::objectResponse( $session,
        \&_dateRangeSearch );
}
...
sub _getEventForm {
    ...
    if ($form) {
        ...
        $response->{eventForm} =
          Foswiki::Func::renderText(
            Foswiki::Func::expandCommonVariables($form) );
    }
    ...
    $response->{elements} =
      Foswiki::Func::renderText(
        Foswiki::Func::expandCommonVariables($elements) );
    $response->{type} = $type;
    return $response;
}
...
sub _handleFullCalendar {
    ...
    $reltopic = ( scalar(@calendars) == 1 )
      && ( $rw . '.' . $rt eq $calendartopic ) ? '' : $rw . '.' . $rt;
    ...
    Foswiki::Func::addToZone( 'head', 'FCP_CSS', $fcpCSS,
        'JQUERYPLUGIN::THEME' );
    Foswiki::Func::addToZone( 'script', 'FCP_JS', $fcpJS, 'HIJAXPLUGIN_JS' );
    Foswiki::Func::addToZone( 'script', 'calendar' . $rand,
        <<"HERE", 'FCP_JS' );
...
/(\d\d\d\d)(?:-(\d\d)(?:-(\d\d))?)?(?:T(\d\d)(?::(\d\d)(?::(\d\d(?:\.\d+)?))?)?)?(Z|[-+]\d\d(?::\d\d)?)?/
      )
    {
        ...
        $dateObj->{days} =
          Date_to_Days( $dateObj->{Y}, $dateObj->{M}, $dateObj->{D} );
        return $dateObj;
    }
    return undef;
}
...
sub getMatchingActions {
    ...
    my $due =
      '<' . sprintf( '%.4u/%.2u/%.2u', $end->{Y}, $end->{M}, $end->{D} );
    my $attrs = {
        web   => $web,
        topic => 'WebActions',
        due   => $due,
    ...
    my $actions =
      Foswiki::Plugins::ActionTrackerPlugin::ActionSet::allActionsInWeb( $web,
        $attrs, 0 );
...
sub _dateRangeSearch {
    ...
    foreach (@calendars) {
        ...
        my $events =
          Foswiki::Plugins::FullCalendarPlugin::EventSet::dateRangeSearch( $cw,
            $ct, $start, $end, $reltopic );
        $es->concat($events);
    }
    ...
    # the chronological order of the pairs is important
        if ( $startObj->{D} == 1 ) {
            my ( $prevY, $prevM, $prevD ) =
              Add_Delta_Days( $startObj->{Y}, $startObj->{M}, 1, -1 );
...
# we're looking at a month view with multiple months in view, let's add the middle/main month to the ympair array
        ...
        # writeDebug(Dumper($startObj));
        $es =
          $es->expandEvents( $startObj, $endObj, $query->param('asobject') );
    }
...
sub _eventEdit {
    ...
    try {
        ( $event, $pre, $post, $meta ) =
          Foswiki::Plugins::ObjectPlugin::ObjectSet::loadToFind( $uid, $web,
            $topic, undef, 'CHANGE', 0,
            'Foswiki::Plugins::FullCalendarPlugin::Event' );
    }
...
# - like a mini dateRangeSearch
        ...
        $event->expand( $eventSet, $startObj, $endObj,
            $query->param('asobject') );
        ...
        if ( scalar @{ $eventSet->{OBJECTS} } ) {
            ...
            throw Foswiki::OopsException(
                ...
                params => [
                    $uid,
                    "$startObj->{D} $months{$startObj->{M}} $startObj->{Y}"
                ]
            );
        }
    }
