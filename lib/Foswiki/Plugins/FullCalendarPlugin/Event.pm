...
# Foswiki::Plugins::FullCalendarPlugin::writeDebug(Dumper($this)." ".$start." ".$end);
    return 0
      unless $this->{startDate};  # the minimum required to appear on a calendar
    return 1
      if ( ( $this->{startDate} lt $end )
        && ( !$this->{rangeEnd} || ( $this->{rangeEnd} ge $start ) ) );
    return 0;
}
...
sub setFullCalendarAttrs {
    ...
    $this->{allDay} += 0;
    $this->{title} ||=
      'no title';    # one of the required fields in an FC Event Object
}
...
# Foswiki::Plugins::FullCalendarPlugin::writeDebug("ndowDays - ".Dumper($this));
    ...
    do {
      ...
      ; # some months will not have a 5th Monday, for example, so give the last Monday in the month
...
# Foswiki::Plugins::FullCalendarPlugin::writeDebug("exceptionInMonth - $this->{exceptions}");
    ...
    if ( $this->{exceptions} ) {
        ...
        foreach my $exception (@exceptions) {
            $exception =~ s/:\d+$//;
            return 1
              if ( $exception ge sprintf( '%.4u-%.2u-%.2u', $Y, $M, 1 )
...
sub yearInSync {
    ...
    if ( $every > 1 ) {
        ...
        my ( $Dy, $Dm, $Dd ) =
          N_Delta_YMD( $date->{Y}, $date->{M}, 1, $Y, $M, 1 );
        return 0 if $Dm;   # fraction of a year so won't match a yearly repeater
        return 1
          if !$Dy
        ; # no year or month deltas so it's the month in which the event was created, match
        return 0
          if $Dy < $every
        ;    # it's a full year but only part way through the repeat cycle
        return 1
          if ( $Dy % $every ) == 0;    # if no remainder, then a yearly match
        return 0;                      # no match
    }
    return 1;
}
...
# Foswiki::Plugins::FullCalendarPlugin::writeDebug("monthInSync: return 0 if $Dd < 0 || $Dm < 0");
        return 0
          if $Dd < 0
              || $Dm <
              0; # it's the month before the first date of the event so no match
...
sub addIfInView {
    ...
    Foswiki::Plugins::FullCalendarPlugin::writeDebug(
        "addIfInView: $this->{repeater} $start->{days} <= $days < $end->{days}"
    );
    if (   ( $days >= ( $start->{days} - $this->{durationDays} ) )
        && ( $days < $end->{days} ) )
    {
...
sub expand {
    ...
    if ( $this->{rangeEnd} ) {
        $this->{parsedRange} =
          Foswiki::Plugins::FullCalendarPlugin::parseISOdate(
            $this->{rangeEnd} )
          unless defined $this->{parsedRange};
        $end =
            $this->{parsedRange}->{days} > $end->{days}
          ? $end
          : $this->{parsedRange};
    }
    my ( $Y, $M, $D );
    ...
    Foswiki::Plugins::FullCalendarPlugin::writeDebug(
        "$repeater $start->{date} $end->{date} $this->{exceptions}");
...
# stretch out the start of the view period to catch any nearby events which span over days
            if ( $durationDays
                && ( ( $startDays - $date->{days} ) > $durationDays ) )
            {
                ( $Y, $M, $D ) =
                  Add_Delta_Days( $Y, $M, $D, 0 - $durationDays );
                $period += $durationDays;
     ...
     # Foswiki::Plugins::FullCalendarPlugin::writeDebug("$repeater $Y, $M, $D");
            if ( $every > 1 ) {    # && ($startDays - $date->{days}) > 7) {
                my $dd =
                  Day_of_Week( $date->{Y}, $date->{M}, $date->{D} )
                  ;                # Mon = 1 .. Sun = 7
                my $monday =
                  $date->{days} -
                  --$dd;    # monday of the week in which the event starts
        ...
        Foswiki::Plugins::FullCalendarPlugin::writeDebug(
            "$repeater $Y $M $D period $period startdow $dow1 dow $dow");
        for ( my $day = 0 ; $day < $period ; ) {
            ...
            for ( ; $dow1 <= 7 ; $dow1++ ) {
                ...
                if ( $dow =~ /$dow1/ ) {
                    ...
                    Foswiki::Plugins::FullCalendarPlugin::writeDebug(
"exceptions $exceptions date $date startdate $start->{date}"
                    );
...
# next if $date lt $start->{date};
            ...
            $day =
              $i +
              $jump -
              7; # done like this because dow1 does not always start on 1 (=Mon)
            $dow1 = 1;
        }
        return;
        ...
        my $rem = 0;
        if ( $date->{days} >= $start->{days} )
        {    # first occurence of event is within view window
            $period = $end->{days} - $date->{days};
            ( $Y, $M, $D ) = ( $date->{Y}, $date->{M}, $date->{D} );
        }
        else {
...
# slide the start of the view period to match any nearby events which span over days
            ...
            ( $Y, $M, $D ) =
              Add_Delta_Days( $start->{Y}, $start->{M}, $start->{D}, 0 - $rem );
            $period += $rem;
