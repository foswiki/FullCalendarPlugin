...
function getForm() {
	foswiki.HijaxPlugin.serverAction({
		...
		loading: false,
		nooverlay: true,
		url: foswiki.getPreference('SCRIPTURL')+'/rest/FullCalendarPlugin/form',
...
function initForm(html) {
	...
	oproot = foswiki.getPreference('SCRIPTURL')+'/rest/ObjectPlugin/';
	...
	$form.find("input[name='category']").click(function() {
		...
		switch (thisValue) {
			case "leave":
		                $apptdetails.hide();
				$("#appt_users").attr({'multiple':false,'size':1}).val(foswiki.getPreference('WIKINAME'));
	...
	$('#appt_form_submit').click(function(){
		// create a new event
		foswiki.HijaxPlugin.serverAction({
			url: foswiki.getPreference('SCRIPTURL')+'/rest/ObjectPlugin/new',
...
function getEvent(event,func) {
	...
	foswiki.HijaxPlugin.serverAction({
		type: "GET",
		async: false,
		url: foswiki.getPreference('SCRIPTURL')+'/rest/FullCalendarPlugin/edit',
		data: {
...
init : function(cal,calendartopic,reltopic,viewall) {
	if (calendartopic.search(/\./) == -1) calendartopic = foswiki.getPreference('WEB')+'.'+calendartopic; // SMELL: a bit simple
    $(cal).mouseup(function(){
        ...
        events: function(start, end, reportEventsAndPop) {
			foswiki.HijaxPlugin.serverAction({
				url: foswiki.getPreference('SCRIPTURL')+'/rest/FullCalendarPlugin/events',
				type: 'GET',
				async: true,
